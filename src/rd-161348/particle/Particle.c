...
void particle_create(Particle* particle, Level* level, float x, float y, float z, float xa, float ya, float za, int tex) {
    ...
    particle->xd = particle->xd / dd * speed * 0.4F;
    particle->yd = particle->yd / dd * speed * 0.4F + 0.1F;
    particle->zd = particle->zd / dd * speed * 0.4F;
    particle->uo = (float)frand * 3.0F;
    particle->vo = (float)frand * 3.0F;
    particle->size = (float)(frand * 0.5 + 0.5);
    particle->lifetime = (int)(4.0 / (frand * 0.9 + 0.1));
    particle->age = 0;
}
...
void particle_tick(Particle* particle) {
    ...
    particle->yd = (float)((double)particle->yd - 0.04);
...
void particle_render(Particle* particle, float a, float xa, float ya, float za, float xa2, float za2) {
    ...
    tesselator_vertexUV(x - xa * r - xa2 * r, y - ya * r, z - za * r - za2 * r, u0, v1);
    tesselator_vertexUV(x - xa * r + xa2 * r, y + ya * r, z - za * r + za2 * r, u0, v0);
    tesselator_vertexUV(x + xa * r + xa2 * r, y + ya * r, z + za * r + za2 * r, u1, v0);
    tesselator_vertexUV(x + xa * r - xa2 * r, y - ya * r, z + za * r - za2 * r, u1, v1);
}
