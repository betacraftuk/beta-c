...
void player_tick(Player* player) {
    ...
    if (player->keys.isSpace && player->entity.onGround) {
        player->entity.yd = 0.5f;
    }

    entity_moveRelative(&player->entity, xa, ya, player->entity.onGround ? 0.1f : 0.02f);
    player->entity.yd = (player->entity.yd - 0.08);
    ...
    if (player->entity.onGround) {
        player->entity.xd *= 0.7F;
        player->entity.zd *= 0.7F;
    }
}
